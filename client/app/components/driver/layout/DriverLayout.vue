  ...
  <div class="min-h-screen bg-gray-50 flex flex-col max-w-md mx-auto relative">
    <header class="bg-white border-b border-gray-100 px-4 py-4 flex items-center justify-between sticky top-0 z-30 shadow-sm">
      ...
      <div class="flex items-center gap-3">
        <div class="flex items-center gap-2">
          ...
          <button @click="isOnline = !isOnline" :class="['relative w-11 h-6 rounded-full transition-colors duration-200', isOnline ? 'bg-green-500' : 'bg-gray-300']">
        ...
        <button class="relative w-9 h-9 bg-gray-100 rounded-full flex items-center justify-center">
          ...
          <span class="absolute -top-0.5 -right-0.5 w-4 h-4 bg-red-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center">2</span>
        </button>
      </div>
    </header>
    ...
    <nav class="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md bg-white border-t border-gray-100 shadow-lg z-30">
      <div class="flex items-center justify-around px-2 py-2">
        <NuxtLink v-for="item in navItems" :key="item.to" :to="item.to" class="flex flex-col items-center gap-1 px-4 py-2 rounded-xl transition-colors" :class="$route.path === item.to ? 'text-green-500' : 'text-gray-400'">
